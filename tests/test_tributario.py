...
# --------------------------------------------------------------------------- #
# INSS patronal — o item que faltava e invertia a conclusão                    #
# --------------------------------------------------------------------------- #
def test_sem_folha_o_presumido_parece_mais_barato():
    """Documenta o comportamento incompleto de propósito.

    Ignorar a contribuição patronal — que está DENTRO do DAS no Simples e FORA
    no Presumido — fazia a ferramenta recomendar Lucro Presumido para um
    minimercado típico. Este teste fixa o número para que a diferença com o
    caso seguinte fique explícita.
    """
    r = comparar_regimes(35_000, 55, folha_mensal=0)
    assert r.presumido < r.simples_otimizado
    assert r.detalhamento_presumido["INSS patronal"] == 0
    assert r.melhor_regime == "Lucro Presumido"


def test_com_folha_realista_o_simples_vence():
    """A conclusão correta: minimercado com 3 funcionários (~R$ 6 mil)."""
    r = comparar_regimes(35_000, 55, folha_mensal=6_000)
    assert "Simples" in r.melhor_regime
    assert r.simples_otimizado < r.presumido
...
def test_existe_folha_de_virada():
    """A virada é contínua, não um caso isolado."""
    virou = None
    for folha in range(0, 12_001, 250):
        r = comparar_regimes(35_000, 55, folha_mensal=folha)
        if r.simples_otimizado < r.presumido:
            virou = folha
            break
    assert virou is not None and 0 < virou < 6_000


def test_anexo_iv_soma_o_encargo_nos_dois_lados():
    """Construção civil e vigilância recolhem CPP por fora do DAS também.

    Se o encargo entrasse só de um lado, o Anexo IV ficaria artificialmente
...
def test_segregacao_nao_mexe_no_encargo_de_folha():
    a = comparar_regimes(35_000, 0, folha_mensal=6_000)
    b = comparar_regimes(35_000, 90, folha_mensal=6_000)
    assert a.encargo_folha_anual == pytest.approx(b.encargo_folha_anual)
    assert b.economia_monofasico > a.economia_monofasico
