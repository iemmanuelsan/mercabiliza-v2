...
# O render.yaml deste repositório é o blueprint da API DE DOCUMENTOS
# (Dockerfile.api). O app Streamlit fica no Streamlit Community Cloud, e por
# isso não há serviço nem banco declarados para ele aqui — declarar criaria um
# segundo serviço pago sem necessidade. O caminho de mover o Streamlit para o
# Render está documentado em DEPLOY.md.
def test_render_declara_a_api_em_docker(render):
    servico = render["services"][0]
    assert servico["runtime"] == "docker"
    assert servico["dockerfilePath"] == "./Dockerfile.api", (
        "o blueprint tem de apontar para o Dockerfile da API, não para o do "
        "Streamlit — este repositório tem os dois"
    )


def test_render_nao_declara_banco(render):
    """A API é sem estado: quem guarda dado é o Supabase, via app web.

    Declarar um Postgres aqui criaria um banco pago que ninguém usa.
    """
    assert "databases" not in render or not render.get("databases")


def test_render_nao_versiona_nenhum_segredo(render):
    """Toda variável de chave precisa de sync:false — o arquivo vai pro Git."""
...
def test_render_exige_a_chave_da_api(render):
    """Sem DOCUMENTOS_API_KEY declarada, o serviço sobe e recusa tudo (503)."""
    chaves = {e["key"] for e in render["services"][0]["envVars"]}
    assert "DOCUMENTOS_API_KEY" in chaves


def test_render_healthcheck_bate_com_o_dockerfile_da_api(render):
