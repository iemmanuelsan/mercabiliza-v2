...
def gerar_dossie(empresa: Empresa) -> bytes:
    ...
    pdf.linha(
        5,
        f"Situação: {empresa.situacao.situacao_receita} | "
    ...
    if empresa.socios:
        ...
        if empresa.tem_risco_societario:
            ...
            pdf.paragrafo(
                4,
                "ALERTA: múltiplos sócios. Verificar participação ≥10% em "
    ...
    pdf.paragrafo(
        4, f"Emitido em {date.today():%d/%m/%Y} | Fontes: {', '.join(empresa.fontes) or 'n/d'}"
    )
    return pdf.bytes()


def gerar_proposta(empresa: Empresa, honorarios: Honorarios, incluir_dp: bool = True) -> bytes:
    ...
    pdf.linha(
        5,
        f"Cidade/UF: {empresa.endereco.municipio}/{empresa.endereco.uf} "
    ...
    pdf.fonte("", 9)
    pdf.linha(
        5, f"• Honorários recorrentes (mensalidade base): {moeda(honorarios.base)} / mês"
    )
    if honorarios.adicional_cnpjs > 0:
        pdf.linha(
            5, f"• Adicional por unidades/CNPJs: {moeda(honorarios.adicional_cnpjs)} / mês"
        )
    if honorarios.adicional_dp > 0:
        pdf.linha(
            5,
            f"• Adicional Departamento Pessoal ({honorarios.blocos_dp} bloco(s)): "
    ...
    pdf.linha(
        4,
        f"Proposta emitida em {date.today():%d/%m/%Y} — válida por "
