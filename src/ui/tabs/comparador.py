...
def render() -> None:
    ...
    if folha <= 0:
        st.warning(
            "⚠️ **Com folha zerada a comparação fica incompleta.** No Simples o "
            "INSS patronal está dentro do DAS; no Lucro Presumido é recolhido "
            "por fora, a ~27,8% da folha. Sem informar a folha, o Presumido "
            "aparece mais barato do que é — e numa empresa com funcionários a "
            "conclusão chega a inverter."
        )

    resultado = comparar_regimes(faturamento, pct_mono, folha_mensal=folha)
    ...
    with dir_:
        ...
        st.caption(
            ...
            "recolhido por fora do imposto. Não inclui ICMS, ISS nem FGTS — o "
            "FGTS incide igual nos dois regimes."
        )
