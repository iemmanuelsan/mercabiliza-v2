...
class Endereco:
    ...
    @property
    def complemento_limpo(self) -> str:
        """Complemento da Receita normalizado para uso em documento.

        O campo vem sujo da base da Receita Federal, e é lixo de digitação de
        quem preencheu o cadastro — não erro nosso. Um caso real:

            "CASA CASA CASA CASA ;CASA TERREO ;CASA TERREO"

        que saía assim mesmo no contrato, com o ``;`` colado na palavra
        seguinte e a repetição toda. Três limpezas, nesta ordem:

        1. **Separadores.** O ``;`` vira vírgula com espaço. É de onde vinha o
           "palavra ;palavra".
        2. **Palavra repetida em sequência.** "CASA CASA CASA CASA" vira
           "CASA". Só colapsa repetição CONSECUTIVA: "SALA 2 SALA 3" continua
           inteiro, porque as palavras iguais não estão coladas.
        3. **Trecho contido em outro.** Se as palavras de um trecho são um
           subconjunto das de outro, o menor sai: {CASA} ⊂ {CASA, TERREO},
           então "CASA, CASA TERREO" vira "CASA TERREO".

        A comparação do item 3 é por CONJUNTO DE PALAVRAS, não por prefixo de
        texto. Por prefixo, "SALA 1" seria engolido por "SALA 10" — que são
        endereços diferentes.
        """
        bruto = (self.complemento or "").strip()
        if not bruto:
            return ""
        ...
        for pedaco in re.split(r"[;,]", bruto):
            ...
            # (2) colapsa repetição consecutiva
            enxuto: list[str] = []
            for palavra in palavras:
                if not enxuto or enxuto[-1].casefold() != palavra.casefold():
                    enxuto.append(palavra)
            trechos.append(" ".join(enxuto))

        # (3) remove trecho cujas palavras já estão contidas em outro
        finais: list[str] = []
    ...
    def logradouro_numero(self) -> str:
        ...
        complemento = self.complemento_limpo
    ...
    def linha_juridica_negrito(self) -> str:
        """Endereço com logradouro, bairro, cidade e CEP em negrito.

        Marcadores ``**`` — o gerador de PDF converte para ``<b>``.

        A CIDADE passou a sair em negrito na revisão de 01/09/2026. Antes ela
        ficava em texto normal no meio de um endereço todo destacado, o que
        deixava justamente o dado de foro e competência menos visível que o
        número da rua. Quem confere um contrato procura a cidade.
        """
        partes = []
