...
# [CORRIGIDO] Contribuição patronal sobre a folha — o item que faltava e que
# INVERTIA a conclusão do comparador.
#
# No Simples (Anexos I, II, III e V) o INSS patronal está DENTRO do DAS. No
# Lucro Presumido é guia separada:
#     CPP        20,0%  sobre a folha
#     RAT         ~2,0%  varia de 1% a 3% pelo risco da atividade
#     Terceiros   ~5,8%  SESC, SENAC, SEBRAE, INCRA, salário-educação
#     ------------------
#     Total      ~27,8%
#
# Sem isso, para um minimercado de R$ 35 mil/mês com 55% de monofásico a
# função dizia "Lucro Presumido" (R$ 16.474 contra R$ 23.820 do Simples). Com
# uma folha de R$ 6 mil/mês o Presumido vai a R$ 36.490 e o Simples ganha com
# folga. Era recomendação errada, com dinheiro real em cima.
#
# O FGTS (8%) fica FORA de propósito: incide igual nos dois regimes, então
# entrar na conta só inflaria os dois lados sem mudar a comparação.
ALIQ_CPP_PATRONAL = 0.20
ALIQ_RAT_PADRAO = 0.02
ALIQ_TERCEIROS = 0.058
ENCARGO_FOLHA_FORA_DO_SIMPLES = ALIQ_CPP_PATRONAL + ALIQ_RAT_PADRAO + ALIQ_TERCEIROS
...
class ComparacaoRegimes:
    ...
    encargo_folha_anual: float = 0.0
    cpp_fora_do_simples: bool = False
...
def comparar_regimes(
    ...
    ``folha_mensal`` **precisa ser informada** para a comparação valer: no
    Simples o INSS patronal está dentro do DAS, no Presumido é guia separada
    de ~27,8% sobre a folha. Sem ela, o Presumido parece mais barato do que é
    e a conclusão chega a inverter numa empresa com funcionários.

    ``cpp_fora_do_simples`` deve ser ``True`` no Anexo IV (construção civil e
    vigilância), o único em que a contribuição patronal também é recolhida por
    fora do DAS — nesse caso o encargo entra nos DOIS lados e volta a não
    diferenciar.
    """
    ...
    encargo_folha_anual = folha * 12 * ENCARGO_FOLHA_FORA_DO_SIMPLES

    # --- Simples Nacional -------------------------------------------------- #
    aliq_simples = aliquota_efetiva(fat_anual, tabela)
    simples_das = fat_anual * aliq_simples

    # No Anexo IV a CPP fica fora do DAS: entra também deste lado.
    encargo_no_simples = encargo_folha_anual if cpp_fora_do_simples else 0.0
    ...
    return ComparacaoRegimes(
        ...
        folha_mensal=folha,
        encargo_folha_anual=encargo_folha_anual,
        cpp_fora_do_simples=cpp_fora_do_simples,
    )
